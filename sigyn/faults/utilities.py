...
def scramble(collection, scrambledness, confusability_range):
    new_collection = []
    visited_indices = set()
    for index, item in enumerate(collection):
        if random.random() <= scrambledness:
            index = int(abs(index+(confusability_range*(random.random()-0.5))))
...
def dropped_scramble(collection, scrambledness, confusability_range):
    return [i for i in scramble(collection, scrambledness, confusability_range) if random.random() > scrambledness/10]
